...
impl Crane {
    fn parse(i: &str) -> IResult<&str, Self> {
        let (i, (_, ax, _, ay, _)) = tuple(
            (tag("Button A: X+"), nom_u64, tag(", Y+"), nom_u64, line_ending)
        )(i)?;
        let (i, (_, bx, _, by, _)) = tuple(
            (tag("Button B: X+"), nom_u64, tag(", Y+"), nom_u64, line_ending)
        )(i)?;
        let (i, (_, px, _, py, _)) = tuple(
            (tag("Prize: X="), nom_u64, tag(", Y="), nom_u64, line_ending)
        )(i)?;
        Ok((i, Crane {
            a: (ax, ay),
            b: (bx, by),
    ...
    fn matrix(&self) -> Option<[Fraction; 4]> {
        let a: [Fraction; 4] = [self.a.0.into(), self.a.1.into(), self.b.0.into(), self.b.1.into()];
